...
const database1 = require("./PropertyAdvance")//get property
const database = require("./Zone(1-19).js");//get zone
const Winners = require("./propertywinners");//post winners
const ExcelJS = require("exceljs");//excel
const Author =  require("./Author");//get author
const Water = require("./WaterAdvance") //get water
const WaterWinners = require("./waterwinners")//post water
...
app.post("/first/:srno", async (req, resp) => {
  ...
  await Winners.updateOne({ SR_NO: srno }, { $set: { POSITION: "1st winner" } });
  resp.send(result);
});
...
app.post("/second/:srno", async (req, resp) => {
  const srno = req.params.srno;
  let result = await Winners.insertMany(req.body);
  await Winners.updateMany({ SR_NO: srno }, { $set: { POSITION: "2nd winner" } });
  resp.send(result);
});
...
app.post("/third/:srno", async (req, resp) => {
  const srno = req.params.srno;
  let result = await Winners.insertMany(req.body);
  await Winners.updateMany({ SR_NO: srno }, { $set: { POSITION: "3rd winner" } });
  resp.send(result);
});
...
app.post("/Forth/Zone1/:srno", async (req, resp) => {
  const srno = req.params.srno;
  let result = await Winners.insertMany(req.body);  
  await Winners.updateMany({ SR_NO: srno }, { $set: { POSITION: "Zone 1 winner" } });
  resp.send(result);
});
...
app.post("/Forth/Zone2/:srno", async (req, resp) => {
  const srno = req.params.srno;
  let result = await Winners.insertMany(req.body);
  await Winners.updateMany({ SR_NO: srno }, { $set: { POSITION: "Zone 2 winner" } });
  resp.send(result);
});
...
app.post("/Forth/Zone3/:srno", async (req, resp) => {
  const srno = req.params.srno;
  let result = await Winners.insertMany(req.body);
  await Winners.updateMany({ SR_NO: srno }, { $set: { POSITION: "Zone 3 winner" } });
  resp.send(result);
});
...
app.post("/Forth/Zone4/:srno", async (req, resp) => {
  const srno = req.params.srno;
  let result = await Winners.insertMany(req.body);
  await Winners.updateMany({ SR_NO: srno }, { $set: { POSITION: "Zone 4 winner" } });
  resp.send(result);
});
...
app.post("/Forth/Zone5/:srno", async (req, resp) => {
  const srno = req.params.srno;
  let result = await Winners.insertMany(req.body);
  await Winners.updateMany({ SR_NO: srno }, { $set: { POSITION: "Zone 5 winner" } });
  resp.send(result);
});
...
app.post("/Forth/Zone6/:srno", async (req, resp) => {
  const srno = req.params.srno;
  let result = await Winners.insertMany(req.body);
  await Winners.updateMany({ SR_NO: srno }, { $set: { POSITION: "Zone 6 winner" } })
  resp.send(result);
});
...
app.post("/Forth/Zone7/:srno", async (req, resp) => {
  ...
  await Winners.updateMany({ SR_NO: srno }, { $set: { POSITION: "Zone 7 winner" } });
  resp.send(result);
});
...
app.post("/Forth/Zone8/:srno", async (req, resp) => {
  const srno = req.params.srno;
  let result = awaitWinners.insertMany(req.body);
  await Winners.updateMany({ SR_NO: srno }, { $set: { POSITION: "Zone 8 winner" } });
  resp.send(result);
});
...
app.post("/Forth/Zone9/:srno", async (req, resp) => {
  const srno = req.params.srno;
  let result = awaitWinners.insertMany(req.body);
  await Winners.updateMany({ SR_NO: srno }, { $set: { POSITION: "Zone 9 winner" } });
  resp.send(result);
});
...
app.post("/Forth/Zone10/:srno", async (req, resp) => {
  const srno = req.params.srno;
  let result = await Winners.insertMany(req.body);
  await Winners.updateMany({ SR_NO: srno }, { $set: { POSITION: "Zone 10 winner" } });
  resp.send(result);
});
...
app.post("/Forth/Zone11/:srno", async (req, resp) => {
  const srno = req.params.srno;
  let result = await Winners.insertMany(req.body);
  await Winners.updateMany({ SR_NO: srno }, { $set: { POSITION: "Zone 11 winner" } });
  resp.send(result);
});
...
app.post("/Forth/Zone12/:srno", async (req, resp) => {
  const srno = req.params.srno;
  let result = await Winners.insertMany(req.body);
  await Winners.updateMany({ SR_NO: srno }, { $set: { POSITION: "Zone 12 winner" } });
  resp.send(result);
});
...
app.post("/Forth/Zone13/:srno", async (req, resp) => {
  const srno = req.params.srno;
  let result = await Winners.insertMany(req.body);
  await Winners.updateMany({ SR_NO: srno }, { $set: { POSITION: "Zone 13 winner" } });
  resp.send(result);
});
...
app.post("/Forth/Zone14/:srno", async (req, resp) => {
  const srno = req.params.srno;
  let result = await Winners.insertMany(req.body);
  await Winners.updateMany({ SR_NO: srno }, { $set: { POSITION: "Zone 14 winner" } });
  resp.send(result);
});
...
app.post("/Forth/Zone15/:srno", async (req, resp) => {
  ...
  await Winners.updateMany({ SR_NO: srno }, { $set: { POSITION: "Zone 15 winner" } });
  resp.send(result);
});
...
app.post("/Forth/Zone16/:srno", async (req, resp) => {
  const srno = req.params.srno;
  let result = await Winners.insertMany(req.body);
  await Winners.updateMany({ SR_NO: srno }, { $set: { POSITION: "Zone 16 winner" } });
  resp.send(result);
});
...
app.post("/Forth/Zone17/:srno", async (req, resp) => {
  const srno = req.params.srno;
  let result = await Winners.insertMany(req.body);
  await Winners.updateMany({ SR_NO: srno }, { $set: { POSITION: "Zone 17 winner" } });
  resp.send(result);
});
...
app.post("/Forth/Zone18/:srno", async (req, resp) => {
  const srno = req.params.srno;
  let result = await Winners.insertMany(req.body);
  await Winners.updateMany({ SR_NO: srno }, { $set: { POSITION: "Zone 18 winner" } });
  resp.send(result);
});
...
app.post("/Forth/Zone19/:srno", async (req, resp) => {
  const srno = req.params.srno;
  let result = await Winners.insertMany(req.body);
  await Winners.updateMany({ SR_NO: srno }, { $set: { POSITION: "Zone 19 winner" } });
  resp.send(result);
});
...
app.get("/GenerateExcel", async (req, resp) => {
  try {
    ...
    resp.setHeader("Content-Disposition", "attachment; filename=mongodb-data.XLSX");
...
app.post("/waterfirst/:srno", async(req,resp)=>{
  ...
  await WaterWinners.updateMany({ SR_NO: srno }, { $set: { POSITION: "1st winner" } });
  resp.send(result);
...
app.post("/watersecond/:srno", async(req,resp)=>{
  const srno = req.params.srno;
  let result = await WaterWinners.insertMany(req.body);
  await WaterWinners.updateMany({ SR_NO: srno }, { $set: { POSITION: "2nd winner" } });
  resp.send(result);
...
app.post("/waterthird/:srno", async(req,resp)=>{
  const srno = req.params.srno;
  let result = await WaterWinners.insertMany(req.body);  
  await WaterWinners.updateMany({ SR_NO: srno }, { $set: { POSITION: "3rd winner" } });
  resp.send(result);
...
const handleZoneWinner = async (req, resp, zone) => {
  const srno = req.params.srno;
  let result = await WaterWinners.insertMany(req.body);
  await WaterWinners.updateMany({ SR_NO: srno }, { $set: { POSITION: `${zone} winner` } });
  resp.send(result);
};
...
app.post("/Login", async (req, resp) => {
  if (req.body.Password && req.body.Email) {             //user must enter both emil and password for login
