...
function Zone18() {
  ...
  async function GET() {
    ...
    for (let i = 0; i < 5; i++) {
        ...
        localStorage.setItem(`Z${i + 86}`, JSON.stringify(result));
    }
}
  ...
  async function SaveData() {
    try {
        for (const item of state) {
            ...
            const response = await fetch(`http://localhost:5000/Forth/Zone18/${SR_NO}`, {
                method: "POST",
        ...
        navigate("/Property/Forth/Zone19");
    ...
    return (
        ...
        {loading ?
            ...
            <div className="TableDiv1">
                {show ?
                    <h2 className="ZoneHeading">Click To Get Zone 18 Winners </h2>
                    :
                    <h2 className="ZoneHeading"> Zone 18 Winners are </h2>
                }
                 {confettiActive?
    <div>
    ...
    :null
                ...
                <div className="buttons">
                    <div>
                        <Button variant="info" style={{ fontWeight: "700",border:"1px solid black" }} onClick={GET}>Zone 18 Winners</Button>{' '}
                    </div>
                    {save ?
                        <div>
