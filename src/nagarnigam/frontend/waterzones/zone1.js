...
function Zone1() {
    ...
    async function GET() {
        ...
        for (let i = 0; i < 3; i++) {
            ...
            localStorage.setItem(`WZ${i + 1}`, JSON.stringify(result));
        }
    }
    ...
    async function SaveData() {
        try {
            for (const item of state) {
                ...
                const response = await fetch(`http://localhost:5000/WaterForth/Zone1/${SR_NO}`, {
                    method: "POST",
            ...
            {loading ?
                ...
                <div className="TableDiv1">
                    ...
                    <div className="buttons">
                        <div>
                            <Button variant="info" style={{ fontWeight: "700" }} onClick={GET}>Zone 1 Winners</Button>{' '}
                        </div>
                        {save ?
                            <div>
