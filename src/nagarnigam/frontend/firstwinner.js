...
function First({ data }) {
    ...
    async function SaveData1() {
        ...
        const result = await fetch(`http://localhost:5000/first/${record.SR_NO}`, {
    ...
    useEffect(() => {
        if (data1.length > 0 && data1[0].PARTNER) {
            const partnerDigits = Array.from(data1[0].PARTNER.toString()).map(Number);
    ...
    async function GetData1(SR_NO) {
        ...
        if (result) {
            setDATA1(result);
            setsave(true);
            localStorage.setItem("winner1", JSON.stringify(result));
            ...
            <div className="ParentDiv">
                ...
                <div className="Div1">
                    ...
                    <div className="subdivs" >
                        <div ><h2 className="Heading">Property id of the winner is</h2></div>
