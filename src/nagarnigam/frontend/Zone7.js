...
function Zone7() {
  ...
  async function GET() {
    ...
    for (let i = 0; i < 5; i++) {
        ...
        localStorage.setItem(`Z${i + 31}`, JSON.stringify(result));
    }
}
      async function SaveData() {
        try {
            for (const item of state) {
                ...
                const response = await fetch(`http://localhost:5000/Forth/Zone7/${SR_NO}`, {
                    method: "POST",
    ...
    return (
        ...
        {loading ?
            ...
            <div className="TableDiv1">
                {show ?
                    <h2 className="ZoneHeading">Click To Get Zone 7 Winners </h2>
                    :
    ...
    :null
                ...
                <div className="buttons">
                    <div>
                        <Button variant="info" style={{ fontWeight: "700" ,border:"1px solid black"}} onClick={GET}>Zone 7 Winners</Button>{' '}
                    </div>
                    {save ?
                        <div>
